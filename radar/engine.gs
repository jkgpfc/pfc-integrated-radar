...
function pfcBaseFeeds_() {
  return [
    ...
    { tag: 'Coal Gasification', url: gnews_('"coal gasification" OR "underground coal gasification" OR "coal to methanol" OR "coal to SNG" India when:7d') },
    { tag: 'Clean Coal Tech',  url: gnews_('surface coal gasification incentive OR coal liquefaction OR "coal to chemicals" OR oxy-fuel combustion India when:7d') },
...
function pfcSector_(lower, tag) {
  if (/\bcoal\b[^.]{0,30}(gasificat|liquefact|oxidation|to[- ](methanol|sng|liquid|chemical|gas))|gasification (project|plant|complex|unit)|\bucg\b|surface coal gasification|oxy[- ]?fuel/.test(lower)) return 'Coal Gasification';
